...
function onResize()
{
	...
	TweenMax.set(sunburst.node, {transformOrigin:"50% 50%", x: sizes.card.width / 2, y: 0});
...
function makeRain()
{
	...
	var lineLength = currentWeather.type == 'thunder' ? 75 : 35;
	...
	var line = this['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
		fill: 'none',
		stroke: currentWeather.type == 'thunder' ? '#777' : '#0055ff',
...
function makeSplash(x, type)
{
	...
	var splashUp = 0 - (Math.random() * splashBounce * 5);
...
function changeWeather(weather)
{
	...
	switch(weather.type)
	{
		case 'wind':
			...
			break;
		case 'sun':
			TweenMax.to(settings, 3, {windSpeed: 100, ease: Power2.easeInOut});
			break;
		default:
	...
	switch(weather.type)
	{
        ...
        case 'sun':
			TweenMax.to(sun.node, 4, {x: sizes.card.width / 2, y: sizes.card.height / 2, ease: Power2.easeInOut});
			TweenMax.to(sunburst.node, 4, {scale: 2, opacity: 0.8, y: (sizes.card.height/2), ease: Power2.easeInOut});
			break;
		default:
